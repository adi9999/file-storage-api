...
class RestExceptionHandler(
    ...
    fun handleMaxUploadSizeExceeded(ex: MaxUploadSizeExceededException): ResponseEntity<Any> {
        log.warning(ExceptionLogItem("File size exceeded", ex))
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null,
            listOf(ErrorMessage("File size exceeds the maximum allowed limit")),
            HttpStatus.BAD_REQUEST.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), HttpStatus.BAD_REQUEST)
    }

    @ExceptionHandler(AccessDeniedException::class)
    fun handleAccessDenied(ex: AccessDeniedException): ResponseEntity<Any> {
        log.warning(ExceptionLogItem("Access denied", ex))
    ...
    fun handleAll(ex: Exception, request: WebRequest): ResponseEntity<Any> {
        this.log.crit(ExceptionLogItem("Unhandled exception: ${ex.localizedMessage}", ex))
